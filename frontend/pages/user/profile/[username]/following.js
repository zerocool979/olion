/**
 * pages/user/profile/[username]/following.jsx
 * URL: /user/profile/{username}/following
...
export default function FollowingPage() {
  ...
  const followerCnt  = owner?._count?.followers ?? 0
  const followingCnt = owner?._count?.following ?? following.length
  ...
  const sidebar = (
    <div style={{ background: colors.bg, border: `1px solid ${colors.border}`, borderRadius: 16, padding: '14px 16px' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
        ...
        <StatPill icon="👥" value={followerCnt}  label="Pengikut" />
        <StatPill icon="➕" value={followingCnt} label="Mengikuti" accent={colors.accent} />
      </div>
    </div>
  )
  ...
  return (
    <>
      <Head><title>{ownerName} Mengikuti — OLION</title></Head>
      <UserLayout sidebar={sidebar}>
        ...
        <div style={{ display: 'flex', borderBottom: `1px solid ${colors.border}`, position: 'sticky', top: 57, zIndex: 9, background: 'rgba(0,0,0,0.88)', backdropFilter: 'blur(8px)' }}>
          <Link href={ownerName ? `/user/profile/${encodeURIComponent(ownerName)}/followers` : '#'} style={{ flex: 1, display: 'block', textAlign: 'center', padding: '12px 0', fontSize: 14, fontWeight: 400, color: colors.textSecondary, textDecoration: 'none', borderBottom: '2px solid transparent' }}
        ...
        {loading
          ...
          : following.length === 0
          ? <EmptyState icon="➕" title={`${ownerName} belum mengikuti siapapun`} description="Temukan pengguna menarik di halaman Pakar." actionLabel="Jelajahi Pakar" actionHref="/user/experts" Link={Link} />
          : following.map(p => <PersonRow key={p.id} person={p} meId={me?.id} onFollow={handleFollow} followMap={followMap} />)
        }
      </UserLayout>
    </>
  )
